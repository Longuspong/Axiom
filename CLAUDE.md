...
- **Skirmish-Kampfkern gebaut** (Gefechtstyp „Elimination", §5.1) — F5-startbar (`run/main_scene` → `scenes/skirmish.tscn`):
  ...
  - **Steuerung auf EIN Grid-Cursor-Modell umgebaut** (Nutzer-Wunsch „auch mobil testbar", deckt §10.5-Pflichtregel ab): ein Cursor, den Maus, Tastatur (Pfeile/WASD) **und ein virtueller Touch-Joystick + Bestätigen/Zurück-Buttons** gemeinsam bedienen. Statt „Klick = sofort Aktion" öffnet über der aktiven Einheit ein **Aktionsmenü** (Symbol-Chips Bewegen / Angriff bzw. Zauber / Warten), das mit dem Cursor angewählt wird → ermöglicht Hover-Effekte wieder. `scripts/ui/virtual_joystick.gd` (analoge Richtung → zellenweises Schritten mit Repeat), `scripts/ui/action_menu.gd` (schwebende Chips, Cursor-/Maus-Auswahl). `emulate_mouse_from_touch=true` (project.godot) → Grid-Taps funktionieren zusätzlich direkt. Bestätigen: Klick/Enter/Button-A · Zurück: Rechtsklick/Esc/Button-B. **Multitouch (Joystick + Button gleichzeitig) ist bewusst single-pointer-vereinfacht — Feinschliff später am Gerät.**
